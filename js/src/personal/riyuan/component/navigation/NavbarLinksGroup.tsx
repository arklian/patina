...
import {Group, Box, Collapse, Text, UnstyledButton, rem, NavLink, Space} from '@mantine/core';
...
export function NavbarLinksGroup({ icon, label, link, subLinks }: NavbarLinksGroupProps) {
    ...
    <>
      {link ?
        <NavLink
          href={link}
          label={<Text size={'xl'}>{label}</Text>}
          leftSection={<FontAwesomeIcon icon={icon} style={{ width: rem(18), height: rem(18) }} />}
        />
        : renderSubLinkToggle()}
